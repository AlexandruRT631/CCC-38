...
public class Level5 {
    private static int[] findFirstDifferentPoint(int x, int y, char[][] map, int size) {
        Queue<int[]> queue = new LinkedList<>();
        int startValue = map[y][x];

        if (startValue != 'L') {
            // Starting point is not what we're looking for, return it immediately.
            return new int[]{x, y};
        }
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (newX >= 0 && newX < size && newY >= 0 && newY < size && visited[newY][newX] == 0) {
                    if (map[newY][newX] != startValue) {
                        // Found a point different from the starting point
